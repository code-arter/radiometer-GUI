#coding: utf-8
"""--------------------------------------------------------------------
 * $Id: molecular_options.py 3327 2017-12-18 10:13:20Z Claudia.Emde $
 * 
...
class setup_molecular_group():
	...
	def __init__(self):
		...
		atmosphere_file = option(
			...
			tokens= addToken(name='Input.atmosphere_filename', datatype=str, valid_range=['subarctic_winter', 'subarctic_summer', 'midlatitude_summer', 'midlatitude_winter', 'tropics', 'US-standard', file], default='subarctic_winter'),
			parents=['uvspec'], 
		)
		pressure_file = option(
			name='pressure_file',
                        gui_name=u'气压廓线',
			group='atmosphere',
			helpstr='Location of the atmosphere data file.', 
		...
		atmosphere_define = option(
			name='atmosphere_define',
                        gui_name=u'廓线自定义',
			group='atmosphere',
			helpstr='Location of the atmosphere data file.', 
			documentation=documentation['atmosphere_file'], 
			tokens= addToken(name='Input.atmosphere_filename', datatype=str, valid_range=["off", file], default='off'),
			parents=['uvspec'], 
		)
		...
		mol_abs_param = option(
			...
			gui_name=u'分子参数模型',
			group='spectral',
			helpstr=u'分子吸收波段参数化模型',
			documentation=documentation['mol_abs_param'],
			tokens = [ addLogical( gui_name=u"参数化策略", name='Input.ck_scheme', logicals=['kato', 'kato2', 'kato2.96','kato2andwandji','fu','avhrr_kratz','sbdart','lowtran','reptran','reptran_channel','crs',file], setting='CK_'),
		...
		mol_modify = option(
			...
			gui_inputs = ( ListInput(name=u"分子类型", valid_range=[ 'O3','O2','H2O','CO2','NO2','BRO','OCLO','HCHO','O4','SO2','CH4','N2O','CO','N2' ], optional=False),
				       FloatInput(name=u'值', valid_range=[0, 1000000.0]),
				       ListInput(name=u'单位', valid_range=[ 'DU', 'CM_2', 'MM' ], optional=False)),
		...
		mixing_ratio = option(
			...
			gui_inputs = ( ListInput(name=u'分子类型', valid_range=[ 'O2','H2O','CO2','NO2','CH4','N2O','F11','F12','F22' ], optional=False),
				       FloatInput(name=u'值',  valid_range=[0, 1000000.0])),
		...
		self.options = [atmosphere_file,
                                pressure_file,
                                temperature_file,
                                latitude_file,
                                atmosphere_define,
				mixing_ratio, mol_modify,
				pressure,
				mol_abs_param,
				]
