#coding: utf-8
"""--------------------------------------------------------------------
 * $Id: output_options.py 3188 2015-10-07 08:21:16Z josef.gasteiger $
 * 
...
class setup_output_group():
	...
	def __init__(self):
		...
		output_process = option(
			...
			gui_inputs=(ListInput(name='Input.output_unit_processing', valid_range=['integrate', 'sum', 'rgbnorm', 'rgb_norm', 'rgb', 'per_nm', 'per_cm-1', 'per_band', "spectral&sum"], optional=False),),
		...
		filter_function_file = option(
			...
			tokens=[addToken(name='Input.filename[FN_FILTERFUNCTION]',datatype=file, gui_name=u"文件名"),
		...
		mc_backward_output = option(
			...
			tokens=[ addLogical(name='Input.rte.mc.backward.output', logicals=['edir','edn','eup','fdir','fdn','fup','act','abs','emis','heat','exp','exn','eyp','eyn'], setting='MCBACKWARD_', gui_name='output' ),	
