#coding: utf-8
"""--------------------------------------------------------------------
 * $Id: aerosol_options.py 3274 2017-06-30 14:41:18Z Claudia.Emde $
 * 
...
class setup_aerosol_group():
	...
	def __init__(self):
		...
		aerosol_file = option(
			...
			tokens=[ addLogical( name=u'文件类型', logicals=['gg', 'ssa', 'tau', 'explicit', 'moments' ], setting='FN_AER_' ), 
				addToken(gui_name=u"文件名",name=u'Input.aer.filename[id]', datatype=file),
		...
		aerosol_angstrom = option(
			...
			gui_name=u'气溶胶光学厚度(Angstrom formula)',
		        group='aerosol',  
		...
		aerosol_modify = option(	#TODO: valid_ranges for GUI!!
			...
			tokens=[addLogical( name=u'id1', logicals=['gg','ssa', 'tau', 'tau550'], setting='MODIFY_VAR_',gui_name=u'参数' ),
				addLogical( name='id2', logicals=['set', 'scale' ], setting='MODIFY_TYPE_', gui_name=u'范围/设置' ),
				addToken( name='Input.aer.modify[id1][id2]', datatype=float, gui_name=u'值' ),
		...
		aerosol_haze = option(
			...
			gui_inputs=(ListInput(name='Input.aer.vulcan', default=None, valid_range=["Rural-type-aerosols", "Maritime-type-aerosols", "Urban-type-aerosols", "Tropospheric-type-aerosols"], optional=False),),
		...
		aerosol_set_tau_at_wvl = option(
			...
			gui_name=u'气溶胶设置光学厚度',
			group='aerosol',
			helpstr='Set the aerosol optical thickness at lambda (nm)',
			documentation=documentation['aerosol_set_tau_at_wvl'],
                        tokens=[addToken(name='Input.aer.tau_wvl_lambda', datatype=float, default='NOT_DEFINED_FLOAT', valid_range=[0,1e6], gui_name=u'波长'),
				addToken(name='Input.aer.tau_wvl_tau', datatype=float, default='NOT_DEFINED_FLOAT', valid_range=[0,1e6], gui_name=u'光学厚度'),
		...
		aerosol_season = option(
			...
			gui_inputs=(ListInput(name='Input.aer.vulcan', default=None, valid_range=["Spring-summer-profile", "Fall-winter-profile"], optional=False),),
			childs=['aerosol_option_specification'], 
			extra_dependencies=['aerosol_haze','aerosol_vulcan','aerosol_visibility','aerosol_season'],
		)
		...
		aerosol_vulcan = option(
			...
			gui_inputs=(ListInput(name='Input.aer.vulcan', default=None, valid_range=["Background-aerosols", "Moderate-vulcanic-aerosols", "High-vulcanic-aerosols", "Extreme-vulcanic-aerosols"], optional=False),),
			childs=['aerosol_option_specification'], 
			extra_dependencies=['aerosol_haze','aerosol_vulcan','aerosol_visibility','aerosol_season'],
		)
