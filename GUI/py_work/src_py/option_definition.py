...
class option():
	def __init__(self, name=None, group=None, 
		     ...
		     change_speaker=None, speaker=None, enable_values=None, change_values=None,
		     mystic=False, threedmystic=False, islidar=False, developer=False,
		     extra_dependencies=[], 
		     plot=None, showInGui = True,
		     continious_update=False,mandatory=False,non_unique=False, gui_name=None):
		...
		self.childs = childs
		self.name = name
                self.gui_name = gui_name if gui_name is not None else name
		self.help = helpstr
		self.plot = plot
		self.tokens = tokens
		...
		self.dict = {
			...
			'gui_name'       : gui_name,	      #as defined in uvspec_lex.l
			'group'          : group,             # e.g. WC
		...
		self.canChange = self.default_canChange
		if  change_speaker and change_values is not None:
			self.default_speaker = change_speaker
			assert not isinstance(change_values, basestring), "Missing comma in one-item-tuple?"
			self.default_change_func = change_values
			#self.canChange = self._canChangeContinousOption
	...
	def get(self, *args, **kwargs):
        ...
        def change_function_useful(self, mark):
            if mark:
	        self.change_speaker = self.default_speaker
	        self._change_func = self.default_change_func
	        self.canChange = self._canChangeContinousOption
            else:
	        self.change_speaker = None
	...
	def _canEnableContinousOption(self, is_set, get_value):
		...
		if r and is_set(self._speaker) and  \
			...
			return False
	def _canChangeContinousOption(self, is_set, get_value):
		"""
                print "test_name: %s" % self.name
                print "test_func: %s" % self._change_func
                print "test_value: %s" % get_value(self._speaker)
		"""
                new_value = get_value(self.change_speaker)
                if is_set(self.change_speaker):
                    ret_value_list = self._change_func(*new_value)
	...
	def default_canChange(self, is_set, get_value):
                return [] 
