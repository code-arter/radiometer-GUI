#coding: utf-8
"""--------------------------------------------------------------------
 * $Id: cloud_options.py 3244 2016-08-19 10:16:57Z bernhard.mayer $
 * 
...
class setup_cloud_group():
    ...
    def __init__(self):
        ...
        wc_file = option(   #TODO: option 3D only ifthreedmystic
            ...
            tokens = [ addSetting(name='isp', setting=CaothType('wc') ),
                addLogical( name='Input.caoth[isp].source', logicals=ProfileType().get_valid_range(), setting='CAOTH_FROM_', gui_name=u"文件类型"), 
                addToken( name='Input.caoth[isp].filename', datatype=file, gui_name=u"文件名") ],
            parents=['uvspec'],
            childs=['wc_modify',\
                 'interpret_as_level',\
                 'wc_properties',\
	...
	wc_set = option(
		...
		gui_name=u'水云参数',
			group='cloud',
		helpstr=u'设置水云的参数，形成水云文件',
		documentation=documentation['wc_file'],
		gui_inputs=(ListInput(name=u"文件类型", valid_range=["1d", "3d", "ipa_files", "moments"], optional=False),
                            FloatInput(name=u'云底高(km)', default=None, valid_range=[0, 10000000.0]),
		            FloatInput(name=u'云厚度(km)', default=None, valid_range=[0, 10000000.0]),
		            FloatInput(name=u'密度(g/m3)', default=None, valid_range=[0, 10000000.0]),
			    FloatInput(name=u'有效粒子半径(um)', default=None, valid_range=[0, 10000000.0], optional=True)),
                childs=['wc_modify',\
                     'interpret_as_level',\
                     'wc_properties',\
                     'cloudcover'],
        ...
        wc_properties = option(
            ...
            tokens = [ addSetting(name='isp', setting=CaothType('wc') ),
                addLogical( name='Input.caoth[isp].properties', logicals=['hu', 'echam4', 'mie', file], setting='PROP_', gui_name=u'参数化类型'), 
                addLogical( name='Input.caoth[isp].interpolate' , logicals=['interpolate'], optional=True, gui_name=u'插值') ],
            parents=['wc_file', 'wc_set'],
            #parents=['uvspec'],
        )
        ...
        wc_modify = option( #TODO: valid_ranges for GUI!!
            ...
            tokens = [ addSetting(name='isp', setting=CaothType('wc') ),
                addLogical( name='id1', logicals=['gg','ssa', 'tau', 'tau550'], setting='MODIFY_VAR_', gui_name=u'参数' ),
                addLogical( name='id2', logicals=['set', 'scale' ], setting='MODIFY_TYPE_', gui_name=u'范围/设置' ),
                addToken( name='Input.caoth[isp].modify[id1][id2]', datatype=float, gui_name=u'值' ) ],
            parents=['wc_file', 'wc_set'],
            #parents=['uvspec'],
            non_unique=True,
        )
	...
	ic_set = option(
        ...
        ic_properties = option(
            ...
            tokens = [ addSetting(name='isp', setting=CaothType('ic') ),
                                   addLogical( name='Input.caoth[isp].properties', logicals=['fu', 'echam4', 'yang', 'key', 'baum', 'baum_v36', 'hey', 'yang2013', 'raytracing', file], setting='PROP_', gui_name=u'参数化类型' ), 
                        addLogical( name='Input.caoth[isp].interpolate' , logicals=['interpolate'], optional=True, gui_name=u'插值') ],
            parents=['ic_file', 'ic_set'],
            #parents=['uvspec'],
        ...
        cloudcover = option(
            ...
            tokens = [ addToken(name='isp', datatype=CaothType, valid_range=['wc','ic'], gui_name=u"云类型"),
                addToken( name='Input.caoth[isp].cloudcover', datatype=float, valid_range=[0,1], gui_name=u"云覆盖" ) ,
                addSetting( name='Input.caoth[isp].ipa', setting=1 ),
                addSetting( name='Input.ipa', setting=1, default=0 ) ],
            parents=['ic_file','wc_file', 'ic_set', 'wc_set'],
            #parents=['uvspec'],
            non_unique=True,
        )
        ...
        self.options = [wc_file, wc_set, wc_properties, wc_modify, 
                ic_file, ic_set, ic_properties, ic_modify, 
