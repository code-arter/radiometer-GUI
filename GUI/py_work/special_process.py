...
def set_option_value(Options, key, data, count=1):
    if key in Options:
        obj = Options[key]
        input_list = obj.inputs
        for index, input_obj in enumerate(input_list):
            if isinstance(input_obj, widgets.Text):
                input_obj.SetValueNew(" ".join(data[index:]).decode("utf-8"))
            else:
                if index < len(data):
                    input_obj.SetValueNew(data[index].decode("utf-8"))
