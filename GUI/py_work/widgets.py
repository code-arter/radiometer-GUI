...
import src_py.option_definition as OptionDefinition
from src_py.GUI_definition import *
...
class Option(wx.Panel):
	...
	self._mandatory = False
        ...
        self.canChange = option.canChange
        self.true_option = option
        #self.setChange = option.change_function_useful
        #self.true_change = option.canChange
        #self.false_change = option.default_canChange
	...
	self.parent=parent
        self.group=option.get("group")
	self.non_unique=non_unique
	self._is_child=is_child

        # Add name
        name_str = option.gui_name
	...
	self.fontIsSet = wx.Font(10, wx.DEFAULT, wx.NORMAL, wx.BOLD)
        ...
        exclude_list = ["sza", "zout", "phi0", "day_of_year"]
        if len(gui_inputs) == 1 and option.name not in exclude_list:
            ...
            for inp in gui_inputs:
                ...
                #tmp_sizer.Add(tmp_name, 0, wx.RIGHT|wx.ALIGN_RIGHT, border=500)
	...
	self.Childbutton = wx.Button(self, -1, u"添加")
    ...
    def setChange(self, mark):
        self.true_option.change_function_useful(mark)
        self.canChange = self.true_option.canChange
	...
	self.UpdateFont()
        ...
        self.Change(self.canChange(self.is_set_function,
                                       self.get_value_function))

    def AwakeDeps(self):
        if self.name == "main_wave":
            if self.GetValue() == [u"设置波长"]:
                Options["wavelength"].setChange(False)
                Options["wavecount"].setChange(True)
                Options["wavecount"].CanChange()
            else:
                Options["wavecount"].setChange(False)
                Options["wavelength"].setChange(True)
                Options["wavelength"].CanChange()
	...
	out= [self.name + " " + " ".join(value_list)]
	for child in self.childs:
	    if child.IsChanged() and  child.IsSet():
		out[0] += '\n'+child.GetWriteValue()[0]
	return out

    def SpecialEnable(self, enable=True):
	for obj in self.inputs[1:]:
            if not enable:
                obj.Clear()
            obj.Enable(enable)
	for name in self.sub_names[1:]:
            name.Enable(enable)
    ...
    def MultiChoiceEnable(self, enable=True):
	for obj in self.inputs:
		obj.Enable(enable)
    ...
    def Enable(self, enable=True):
        special_name = ["angle_of_pitch", "azimuth_angle", "distance"]
        if self.name in special_name:
            self.SpecialEnable(enable)
            return
        elif self.name == "multi_choice":
            self.MultiChoiceEnable(enable)
            return
	for obj in self.inputs:
		obj.Enable(enable)
	for name in self.sub_names:
    ...
    def Change(self, value=[]):
        if not value or not self.inputs:
            return
        for index, input_value in enumerate(value):
            if index >= len(self.inputs):
                ...
                self.inputs[index].SetValueNew(input_value)
    ...
    def SetValue(self, value):
        if value:
            ...
            try:
                ...
                self.Choice.SetSelection(self.choices.index(value))
	    except:
		try:
...
class OpenButton(Widget):
    ...
    """
    def __init__(self, parent, default=None, buttonlabel=u"打开",
        ...
        Sizer.Add(self.FileInfo, 1, wx.ALIGN_RIGHT|wx.RIGHT, border=10)
        #Sizer.Add(self.FileInfo, 2)
        Sizer.Add(self.Button, 0, flag=wx.RIGHT|wx.ALIGN_RIGHT, border=10)
...
class TextInfo(wx.Panel):
    def __init__(self, parent, text="", border=0, filename="", optional=False):
...
class LoadButtons(wx.Panel):
    """LoadButtons in general page
    
    The design of the loadbuttons.
    """
    def __init__(self, parent, ext_format="*", lbl2="Load",
                 lbl1="Load As", flags=wx.FD_OPEN):
        wx.Panel.__init__(self, parent)
...
class SingleButtons(wx.Panel):
    """LoadButtons in general page
    
    The design of the loadbuttons.
    """
    def __init__(self, parent, ext_format="*",
                 lbl1="Load As", flags=wx.FD_OPEN|wx.FD_OVERWRITE_PROMPT):
        wx.Panel.__init__(self, parent)
...
class NewUvspecProcess(threading.Thread):
    ...
    def run(self):
        ...
        os.remove(self.output_file + ".err")
...
class NewRunUvspec(wx.Frame):
    def __init__(self, parent, log_list, exit_value):
        size = 400, 200
	...
	self.Layout()
        ...
        self.AddToLog(''.join(log_list))
        #self.UvspecFinished(exit_value)
