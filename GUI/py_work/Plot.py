...
class PlotDefine(Plot2D):
    def __init__(self, column_names=["x", "y"], input_list=[], input_dir=None):
        Plot2D.__init__(self, column_names)
        self.column_names.insert(0, u"输出")
        self.input_list = input_list
        self.input_list.insert(0, ["spectral", "integrate"])
        self.ChoiceOption = {}
        self.CheckOption = {}
        self.input_dir = input_dir.decode("utf-8")
        self.text_dict = {u"输出": "output", u"方位角": "phi", u"俯仰角": "umu", u"距离":"distance"}
    ...
    def LayoutFrame(self):
        ...
        for index, name in enumerate(self.column_names):
            ...
            GridSizer.Add(text,
                          flag=wx.ALIGN_LEFT)
            for axis, cls in ("selected", wx.Choice), ("enable", wx.CheckBox):
                if axis == "selected":
                    obj = cls(self.frame.Panel, -1, choices=self.input_list[index])
                    self.ChoiceOption[self.text_dict[name]] = obj
                else:
                    obj = cls(self.frame.Panel, -1)
                    obj.SetValue(1)
                    obj.Enable(False)
                    self.CheckOption[self.text_dict[name]] = obj
                GridSizer.Add(obj, 1, flag=wx.ALIGN_RIGHT)
                self.PlotOption[axis].append(obj)

        self.ChoiceOption["output"].Bind(wx.EVT_CHOICE, self.OnSelect)
    ...
    def OnSelect(self, event):
        output = self.ChoiceOption["output"].GetStringSelection()
        if output == "integrate":
            for name, obj in self.CheckOption.items():
                if name == "output":
                    obj.SetValue(1)
                    obj.Enable(False)
                else:
    ...
    def OnPlotSum(self):
        ...
        key_dict = {}
        select_key = None
        for name, obj in self.ChoiceOption.items():
            if self.CheckOption[name].GetValue() and name != "output":
                select_key = name
            if name == "phi":
                key_dict[name] = obj.GetSelection()
            else:
                key_dict[name] = obj.GetStringSelection()
        print select_key

        if select_key == "phi":
            key_file = "%s_%s_radiance_spectral" % (key_dict["umu"], key_dict["distance"])
            with open(os.path.join(self.input_dir, key_file), "r") as fp:
                line_list = fp.readlines()

            xdata = [float(ele) for ele in self.input_list[1]]
            for line_data in line_list:
                data_list = line_data.split()
                ydata = [float(ele) for ele in data_list[(int(key_dict["phi"]) + 1):]]
        elif select_key == "umu":
            num_dict = {}
            filelist = os.listdir(self.input_dir)
            filelist.sort()
            for filename in filelist:
                if '_' not in filename or filename == "key_path":
                    continue
                umu, distance, radiance, spectral = filename.split('_')
                if distance == key_dict["distance"] and spectral == "integrate":
                    with open(os.path.join(self.input_dir, filename), 'r') as fp:
                        line_list = fp.readlines()
                    for line_data in line_list:
                        data_list = line_data.split()
                        num_dict[float(umu)] = float(data_list[(int(key_dict["phi"]) + 1)])
            xdata = num_dict.keys()
            xdata.sort()  
            ydata = [num_dict[key] for key in xdata]
            
        elif select_key == "distance":
            num_dict = {}
            filelist = os.listdir(self.input_dir)
            filelist.sort()
            for filename in filelist:
                if '_' not in filename or filename == "key_path":
                    continue
                umu, distance, radiance, spectral = filename.split('_')
                if umu == key_dict["umu"] and spectral == "integrate":
                    with open(os.path.join(self.input_dir, filename), 'r') as fp:
                        line_list = fp.readlines()
                    for line_data in line_list:
                        data_list = line_data.split()
                        num_dict[float(distance)] = float(data_list[(int(key_dict["phi"]) + 1)])
            xdata = num_dict.keys()
            xdata.sort()  
            ydata = [num_dict[key] for key in xdata]
        ...
        print ydata
        # Plot
        f = PlotFrame(self.frame, -1, title="%s " % (self._path))
        fig = f.get_figure()
        a = fig.add_subplot(1,1,1)

        #a.plot(xdata, ydata, label=conf["name"])
        a.plot(xdata, ydata, label="integrate")
    
        #a.set_xlabel(x[0]["name"])
        a.set_xlabel(select_key)
        a.legend()
        a.axis("tight")
    ...
    def OnPlot(self, event):
        ...
        for line_data in line_list:
            data_list = line_data.split()
            spectral_val = float(data_list[int(key_dict["phi"]) + 1])
            xdata.append(float(data_list[0]))
            ydata.append(spectral_val)
        ...
        a.plot(xdata, ydata, label="wavelength")
